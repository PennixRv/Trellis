...
This project is a **TypeScript monorepo** using ES modules. It publishes a CLI package (`@pennixrv/trellis`) and a reusable core package (`@pennixrv/trellis-core`). The source code also follows a **dogfooding architecture** - Trellis uses its own configuration files (`.cursor/`, `.claude/`, `.trellis/`) as templates for new projects.
...
```
packages/
├── core/                # @pennixrv/trellis-core: reusable APIs
│   ├── src/
...
└── cli/                 # @pennixrv/trellis: user-facing CLI
    ├── src/
