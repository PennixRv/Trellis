...
The retrieval engine lives in `@pennixrv/trellis-core/mem` (`packages/core/src/mem/`);
...
`commands/mem.ts:parseArgv`, then calls the matching core `@pennixrv/trellis-core/mem`
...
`core/mem/` does **not** use `zod` — `@pennixrv/trellis-core` keeps a
...
### Core — `@pennixrv/trellis-core/mem`
...
- `packages/core/src/mem/index.ts` — `@pennixrv/trellis-core/mem` public surface
