...
> Package boundary and coding rules for `@pennixrv/trellis-core` and the CLI.
...
|---|---|
| `@pennixrv/trellis-core` | Reusable domain logic, storage primitives, reducers, task APIs, channel APIs, and typed contracts. |
| `@pennixrv/trellis` | CLI argument parsing, terminal rendering, command wiring, process exit behavior, template installation, migrations, and release scripts. |
...
```ts
import { createChannelStore } from "@pennixrv/trellis-core/channel";
```
...
```bash
pnpm --filter @pennixrv/trellis-core build
pnpm --filter @pennixrv/trellis typecheck
```
