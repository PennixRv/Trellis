...
`@pennixrv/trellis-core` owns reusable SDK/domain primitives that must stay
...
- CLI code must import core through public exports such as
  `@pennixrv/trellis-core/channel`, not deep paths under `packages/core/src`.
...
```bash
pnpm --filter @pennixrv/trellis-core lint
pnpm --filter @pennixrv/trellis-core typecheck
